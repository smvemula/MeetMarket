...
public extension NSDate{
    ...
    public class func isThisDateYesterday(date : NSDate) -> Bool
    {
        //print("Yesterday Date : \(NSDate.getStartOfDayOnDate(NSDate(timeIntervalSinceNow: -1*24*60*60))) compare Date : \(date)")
        return NSDate.getStartOfDayOnDate(NSDate(timeIntervalSinceNow: -1*24*60*60)).compare(NSDate.getStartOfDayOnDate(date)) == NSComparisonResult.OrderedSame //&& NSDate.getStartOfDayOnDate(NSDate()).compare(date) == NSComparisonResult.OrderedAscending
    }
    
    public func isThisDateToday() -> Bool
    {
        //print("Today Date : \(NSDate()) compare Date : \(date)")
        let greaterThanStart = NSDate.getStartOfDayOnDate(NSDate()).compare(self) == NSComparisonResult.OrderedAscending
        let lesserThanEnd = NSDate.getEndOfDayOnDate(NSDate()).compare(self) == NSComparisonResult.OrderedDescending
